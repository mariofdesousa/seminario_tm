...
$$
\text{dep}_{\text{scr}} = 1 + \frac{1}{n_{dep}\times\log(1-p_{dep})-1},
$$
em que $p_{dep} = n_{dep}/n_{tot}$ é a proporção de palavras relacionadas com depressão do usuário. Pode-se notar que 
$$
p_{dep} \rightarrow 0 \Rightarrow \text{dep}_{\text{scr}} \rightarrow 0 \quad\text{e}\quad p_{dep} \rightarrow 1 \Rightarrow \text{dep}_{\text{scr}} \rightarrow 1,
$$ 
ou seja, quanto mais próximo de um for o valor de $\text{dep}_{\text{scr}}$, mais indícios a pessoa terá de estar escrevendo sobre algo relacionado à sentimentos de depressão. Vale destacar que se a análise fosse feita somente sobre $p_{dep}$, pessoas que escrevem pouco tenderiam a ter mais indícios de depressão mesmo contendo poucas palavras na lista utilizada. Ao se considerar $\text{dep}_{\text{scr}}$ ao invés de simplesmente $p_{dep}$, levamos em conta a ideia de que entre duas pessoas com o mesmo valor de $p_{dep}$, aquela com maior número de palavras depressivas $n_{dep}$ deverá ter um índice de depressão maior, enquante se duas pessoas tiverem o mesmo valor de $n_{dep}$, o valor de $\text{dep}_{\text{scr}}$ seja maior para o indivíduo com maior $p_{dep}$. 

Um problema com $dep_{scr}$ é que, se $p_{dep}$ tende à uma constante conforme $n_{tot}\rightarrow \infty$, então $n_{dep} \rightarrow \infty \Rightarrow \text{dep}_{\text{scr}} \rightarrow 1$, independente do valor limite de $p_{dep}$. Por outro lado, como mencionamos anteriormente, para valores finitos de $n_{tot}$ e $n_{dep}$ temos que $dep_{scr}$ fornece critérios de comparação mais adequados que utilizar somente $p_{dep}$. No código abaixo o valor de $dep_{scr}$ foi calculado para cada usuário analisado.
