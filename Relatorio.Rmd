...
Após criarmos um arquivo com a contagem das palavras, quanto maior for o número de ocorrências de uma palavra, maior a mesma será na nuvem de palavras.  Dessa forma, as wordclouds são feitas com as palavras mais utilizadas em um determinado texto ou documento, de forma que o aspecto visual fique concentrado nas palavras mais importantes.
...
Esse tipo de técnica permite avaliar alterações, caso existam, de sentimentos ao longo do texto. Note que a quantidade de linhas que serão agrupadas depende do tipo e do tamanho do texto que está sendo analisado. Ainda utilizando o dicionário **bing**, apresentamos uma núvem de palavras de sentimentos, isto é, uma nuvem de palavras com distinção entre palavras positivas e negativas. Prosseguimos a análise de sentimentos comparando os três dicionários, de modo a observar se há alguma discrepância em algum deles. 
...
Na análise de contagem de palavras as expressões **depressed**, **depression** e **depressive** são as mais frequentes, como esperado devido ao fato de terem sido as palavras utilizadas como filtro para a seleção dos tweets que compõem a base de dados do trabalho. A fim de se analisar de forma mais precisa a frequência deste subconjunto de dados, optou-se por retirar estas três observações a fim de tornar a escala de frequência da demais palavras mais clara, como pode-se observar abaixo. Para uma análise de dendrograma a contagem de palavras é de grande significância à medida em que os **clusters** são formados a partir das mesmas. Uma das vantagens da construção do dendrograma é o fato de que não é necesessário se definir, a priori, o número de grupos que o conjunto de palavras possui. 

A partir da análise desde dendrograma, optou-se por definir de forma a priori, o número de clusters, de forma a tornar possível sua vizualização de forma mais clara. O método selecionado de cálculo das distâncias entre as observações, no caso a distância entre as contagens, foi o euclidiano. Optou-se por utilizar o dendograma circular por ser mais atrativo em termos visuais e de transmissão de informação. Além disso, definiu-se, a partir da observação do dendrograma original uma subdivisão em pelo menos seis grandes grupos. Avaliado isto, o dendrograma abaixo reflete a aglomeração das 30 palavras mais frequentes da base de dados. Optou-se por separar os grupos em diferentes cores, facilitando a visualização dos clusters. Além disso, as linhas mais grossas expressam os grupos com maior volume de frequência das palavras.
...
dend6 <- term_freq[1:30] %>% dist %>% hclust %>% as.dendrogram %>%
...
A observação do dendrograma revela uma certa dificuldade de identificação de um padrão de aglomeração de palavras. Como exemplo, um mesmo ramo inclui as palavras "sad", "happy", "love" e "day", sendo que "sad" e "happy", palavras com sentidos opostos estão no mesmo subgrupo desde cluster. Pode-se observar, contudo, que há grande número de palavras que expressam sentimentos, especialmente os negativos, estão presentes de um modo geral no dendrograma, evidenciando um padrão de texto com sentido semântico relacionado às expressões pessoais.
