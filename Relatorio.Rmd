...
Sabemos que com a popularização da internet, um enorme volume dados é gerado a cada minuto. Segundo @gomes2013text, o desafio é saber como manipular essa grande quantidade de informação gerada e investigar como as organizações podem se beneficiar dessas informações, considerando que 80% das informações das organizações estão contidas em documentos de texto (vide @Tan99textmining). Uma vez contabilizada a quantidade de palavras que aparecem dentro de um banco de dados pré estabelecido, é possível calcular a freqüência, percentual, das palavras e utilizar o pacote ggplot para elaboração de um histograma para visualização. Outra opção de identificação de concentração de palavras na análise de sentimentos, são as nuvens de palavras, que são imagens que demonstram de maneira visual, a frequência da ocorrência das palavras em um dado texto: quanto maior for o número de ocorrências de uma palavra, maior a mesma será na nuvem de palavras. Depois de criarmos um arquico com a contagem das palavras, devemos intalar um arquivo Word.cloud
...
Para iniciar a análise dos textos sobre depressão provenientes do Twitter faremos uma análise sobre a frequência das palavras. Esse tipo de análise permite identificar quais são os termos mais utilizados pelas pessoas nesse banco de dados, fornecendo uma caracterização inicial do que poderemos encontrar nas análises seguintes. No gráfico acima é apresentado um histograma da frequência de cada termo presente nos dados, com exceção de algumas palavras muito comuns (im, ur, etc.) ou que não temos interesse de incluir nessa análise (depression, depressive, depression). Podemos observar que a frequência possui uma distribuição bastante assimétrica, com a grande maioria das palavras aparecendo em um proporção baixa, enquanto algumas palavras que são mais comuns ficam evidenciadas na cauda do histograma.
...
Como observado no histograma anterior, um pequeno número de palavras possui uma frequência bem maior que as demais, e estas palavras podem fornecer informações sobre o conteúdo dos dados. No gráfico acima, apresentamos as palavras que aparecem com maior frequência nos tweets analisados, com exceção de alguns termos. Podemos observar que boa parte das palavras estão relacionadas à sentimentos, indicando que as palavras relacionadas à depressão podem estar associadas com termos referentes à como as pessoas se sentem. De fato, a palavra mais frequente dentre os termos associados é "feel".
