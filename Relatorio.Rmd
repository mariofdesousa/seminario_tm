...
No trabalho a seguir, utilizamos o R para tratar estes textos e resolver o nosso problema através da Análise de Sentimentos.

## Descrição dos dados
A base de dados utilizada contém 30000 tweets extraídos com auxílio da função $\texttt{searchTwitter}$ do pacote $\texttt{twitteR}$. 

O banco de dados é composto por 10000 tweets escritos em inglês durante o período que compreende 01/01/2015 a 06/06/2017, contendo as palavras "depression", "depressive" e "depressed". 

A função $\texttt{searchTwitter}$ retorna uma lista com diversas informações, então faz-se necessário aplicar a função $\texttt{twListToPdf}$, também disponível no pacote $\texttt{twitteR}$, para transformar a lista do twitter em um dataframe compatível com os métodos  $\texttt{tidy}$ e $\texttt{tm}$.
...
O primeiro passo será realizar a contagem e frequencia das palavras chave escolhidas pelo grupo.
...
Quanto maior for o número de ocorrências de uma palavra, maior a mesma será na nuvem de palavras. Depois de criarmos um arquivo com a contagem das palavras. Para isso, devemos intalar um arquivo Word.cloud
...
Um dos objetivos desse trabalho é fornecer um método que seja capaz de identificar se uma pessoa apresenta sinai de depressão através dos seus textos no Twitter. Uma proposta que apresentamos nessa direção é utilizar um índice que leve em consideração a proporção de palavras relacionadas à depressão que cada usuário digitou. Para isso, consideramos a lista de 99 palavras que possuem alguma relação com depressão fornecidas na página da internet \cite(https://adarkershadeofblue.wordpress.com/2011/08/20/99-words-about-depression/). A palavra depressed também foi adicionada à essa lista, uma vez que na análise de redes essa palavras foi observada possuir uma maior relação com sentimentos depressivos que depressive e depression.

Considerando para cada usuário $n_{tot}$ o número total de palavras digitadas, desconsiderando stopwords, e $n_{dep}$ o número de palavras que estão presentes na lista de 100 palavras relacionadas à depressão, o índice que nós propomos é dado por
$$
\text{dep}_{\text{scr}} = 1 + \frac{1}{n_{\text{dep}}*\log(1-p_{\text{dep}})-1},
$$
em que $p_{dep} = n_{dep}/n_{tot}$ é a proporção de palavras relacionadas com depressão do usuário. Pode-se notar que $p_{dep} \rightarrow 0 \Rightarrow dep_{scr} \rightarrow 0$ e $p_{dep} \rightarrow 1 \Rightarrow dep_{scr} \rightarrow 1$, ou seja, quanto mais próximo de um for o valor de $dep_{scr}$ mais indícios a pessoa tem de estar escrevendo sobre algo relacionado à depressão. Vale destacar que se a análise fosse feita somente sobre $p_{dep}$ pessoas que escrevem pouco tenderiam a ter mais indícios de depressão contendo poucas palavras na lista sobre depressão. Ao considerar $dep_{dep}$ ao invés de simplesmente $p_{dep}$, levamos em conta a ideia de que entre duas pessoas com o mesmo valor de $p_{dep}$, aquela com maior número de palavras depressivas $n_{dep}$ deverá ter um índice de depressão maior, enquante se duas pessoas tiverem o mesmo valor de $n_{dep}$, o valor de $dep_{scr}$ seja maior para o indivíduo com maior $p_{dep}$. 

Um problema com $dep_{scr}$ é que, se $p_{dep}$ tende à uma constante conforme $n_{tot}\rightarrow \infty$, então $n_{dep} \rightarrow \infty \Rightarrow dep_{scr}$, independente do valor limite $p_{dep}$. Por outro lado, como mencionamos anteriormente, para valores finitos de $n_{tot}$ e $n_{dep}$ temos que $dep_{scr}$ fornece critérios de comparação mais adequados que utilizar somente $p_{dep}$. No código abaixo o valor de $dep_{scr}$ foi calculado para cada usuário analisado.
...
Abaixo apresentamos os dois usuários que apresentaram os maiores de $dep_{scr}$ e os dois usuários que apresentaram os menores $dep_{scr}$. Analisando trechos dos textos desses usuários, notamos que os dois primeiros apresentam teores bastante depressivos, com uma alta concentração de palavras negativas, diferentemente dos dois últimos textos, que parecem não serem tão expressivamente negativos como os anteriores.
